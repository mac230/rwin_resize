...
(defun list-line-adder ()
  ...
  (interactive)
  (let* ((list-elt-regex "\\(^    - \\[[[:digit:]]+\\]\\).*\n\\(^ +\\+.*\n\\)*\\(^ .*\n\\)*")
      (non-blank-regex "\\(^[^[:space:]\n]+\\)")
       (reverse-bound
        (save-excursion
...
(defun list-renumber ()
  ...
  (let* ((current-point (point))
         ...
         (reverse-bound
          (save-excursion
            (re-search-backward non-blank nil t)))
         (forward-bound
          (save-excursion
            (re-search-forward non-blank nil t))))
...
(let* ((current-point (point))
       ...
       (prev-list-elt
        (save-excursion
          (re-search-backward list-elt-regex reverse-bound t))))
...
For when calling at the end of a wrapped text sublist line w/ sublists."
  (let ((cp (point))
    ...
    (when (not forward-bound)
      (setq forward-bound (save-excursion
                            (re-search-forward non-blank-line-regex nil t))))
...
(global-set-key (kbd "C-c M-RET") 'mac-org-numbered-list)
